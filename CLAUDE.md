...
- **Environment matters: always run VRT in the container.** The baseline must come from the same image+arch as CI. `Dockerfile.vrt` bakes deps into an image based on `ghcr.io/puppeteer/puppeteer:25.0.4` (Docker layer cache + a BuildKit npm cache mount handle reinstalls — only on `package*.json` change); `docker-compose.vrt.yml` runs it pinned to `platform: linux/amd64` (matching `ubuntu-latest`), bind-mounts the repo and masks the host's macOS-native `node_modules` with the image's baked Linux ones (an empty anonymous volume on `/work/node_modules`). `Dockerfile.vrt` is also the single source of the pinned environment for CI (its `FROM` tag is the only place the puppeteer image version lives) and sets `PUPPETEER_CACHE_DIR=/home/pptruser/.cache/puppeteer` so `npm ci` reuses the browser the base image already bakes (which lives under pptruser, invisible to the root user otherwise) instead of re-downloading Chromium. The bundled Inter font (`packages/lib/src/fonts.ts`) fixes the typeface but **not** the rasterizer — macOS CoreText vs Linux FreeType anti-alias differently, so a natively-captured baseline shows false text diffs against CI.
...
- CI (`.github/workflows/main.yml`): lint → build → screenshot job builds `Dockerfile.vrt` via `docker/build-push-action` with `cache-from/to: type=gha` (layer cache restores baked `node_modules` + base image, so deps aren't reinstalled and Chromium isn't re-downloaded unless `package*.json`/the Dockerfile change), then `docker run`s the image with the workspace bind-mounted (and `/work/node_modules` masked) to run `npm run vrt`, uploads the `.reg` report artifact, and gates on diffs. `update-baseline.yml` is a manual `workflow_dispatch` that builds the same image, captures + promotes inside it, and commits the baseline on the runner.
